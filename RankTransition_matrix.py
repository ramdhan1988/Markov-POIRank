...
def cv_choose_alpha(alpha_set, validation_set, short_traj_set, traj_dict, traj_all, poi_all, poi_cats, poi_clusters, poi_clusters_list, logbins_pop, logbins_visit, logbins_duration):
    ...
    for alpha_i in alpha_set:
        print(f"find best alpha - alpha_i : ", alpha_i)

        scores = []
        for i in range(len(validation_set)):
            tid = validation_set[i]
            te = traj_dict[str(tid)]

            assert(len(te) > 2)

            validation_set = list(map(str, list(validation_set))) # change to int value list
...
# Split data for validation alpa
...
recdict_comb = dict()
cnt = 1
settings = [(PART1.copy(), PART2.copy()), 
            (PART2.copy(), PART1.copy())]
settings_ = 1
for (validation_set, test_set) in settings:
    ...
    alpha_cv = cv_choose_alpha(ALPHA_SET, 
                               ...
                               poi_cats, 
                               poi_clusters=POI_CLUSTERS, 
                               poi_clusters_list=POI_CLUSTER_LIST,
                               logbins_pop=logbins_pop,
                               logbins_visit=logbins_visit, 
                               logbins_duration=logbins_duration)
    
    print('-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')
    print('alpha_cv:', alpha_cv)
    ...
    for i in range(len(test_set)):
        ...
        validation_set = list(map(str, list(validation_set))) # change to int value list
        test_set = list(map(str, list(test_set))) # change to int value list
        ...
        nodes['weight'] = np.log10(nodes['probability'])
        nodes.drop('probability', axis=1, inplace=True)
        comb_dp = find_viterbi(nodes.copy(), 
                               edges.copy(), 
                               te[0], 
                               te[-1],
                               len(te),
        ...
        recdict_comb[str(tid)] = {
            'REAL':te, 
            'REC_RANK_DP':comb_dp, 
            'REC_RANK_ILP':comb_ilp
        }
        
        print(' '*5, 'Combine Rank  DP (Markov):', comb_dp)
        print(' '*5, 'Combine Rank ILP (Markov Path):', comb_ilp)
        sys.stdout.flush()

    json_object = json.dumps(recdict_comb, indent=4)
    with open("result/RankMarkov_Prediction_alpha:"+ str(alpha_cv) +"_cost:"+ str(RANKSVM_COST) +"_" + dat_suffix[dat_ix] +"_"+ str(settings_) +".json", "w") as outfile:
    ...
    for key in sorted(recdict_comb.keys()):
        recall, precision, F1 = calc_F1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_DP'])
        R1_trans.append(recall)
        P1_trans.append(precision)
        F11_trans.append(F1)
        pF11_trans.append(calc_pairsF1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_DP']))
        true_F11_trans.append(true_F1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_DP']))
        true_pF11_trans.append(true_pairsF1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_DP']))

        recall, precision, F1 = calc_F1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_ILP'])
        R2_trans.append(recall)
        P2_trans.append(precision)
        F12_trans.append(F1)
        pF12_trans.append(calc_pairsF1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_ILP']))
        true_F12_trans.append(true_F1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_ILP']))
        true_pF12_trans.append(true_pairsF1(recdict_comb[key]['REAL'], recdict_comb[key]['REC_RANK_ILP']))
    ...
    with open("result/RankMarkov_matric_evaluation_alpha:"+ str(alpha_cv) +"_cost:"+ str(RANKSVM_COST) +"_" + dat_suffix[dat_ix] +"_"+ str(settings_) +".json", "w") as outfile:
